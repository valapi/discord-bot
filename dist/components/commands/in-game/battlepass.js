...
const __command = {
    ...
    async execute({ interaction, language, apiKey, createdTime }) {
        ...
        const AllContracts = (await WebClient.Contract.Fetch(puuid)).data.Contracts;
        ...
        const BP_XpAtNow = BattlePassContract?.ProgressionTowardsNextLevel;
        const BP_TotalEarnedXp = BattlePassContract?.ContractProgression?.TotalProgressionEarned;
        const TheBattlePass = await ValorantApiCom.Contracts.getByUuid(BP_Id);
        const BP_Name = TheBattlePass.data.data?.displayName;
        ...
        const BP_CurrentSlot = TheBattlePass.data.data?.content.chapters[_Slot];
        const BP_LevelSlot = BP_CurrentSlot?.levels[_ofSlot];
        const BP_Slot_ID = BP_LevelSlot?.reward.uuid;
        const BP_Slot_XpNeed = BP_LevelSlot?.xp;
        let BP_Slot_Name = ``;
