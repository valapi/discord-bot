...
const __command = {
    ...
    async execute({ interaction, language, apiKey, createdTime }) {
        ...
        const PlayerMatchHistory = await WebClient.Match.FetchMatchHistory(puuid, interaction.options.getString('queue'));
        ...
        const Match_Display = ThisMap?.listViewIcon;
        ...
        const Player_Kills = ThisPlayer?.stats.kills;
        const Player_Deaths = ThisPlayer?.stats.deaths;
        const Player_Assists = ThisPlayer?.stats.assists;
        const Player_Level = ThisPlayer?.accountLevel;
        const Player_Team = ThisPlayer?.teamId;
        let Player_Rank = ``;
        ...
        const GetAgent = await ValorantApiCom.Agents.getByUuid(ThisPlayer?.characterId);
        ...
        sendMessageArray.at(1)?.addFields({ name: '\u200B', value: '\u200B' }, { name: 'Agent', value: `${Player_Agent_Name}`, inline: true });
        ...
        if (AllMatchData.data.teams.length > 1 && Match_Type !== 'deathmatch') {
            ...
            const Team_isWin = (AllTeams.find(team => team.teamId === Player_Team))?.won;
            if (Team_isWin === true) {
