...
class AxiosClient extends lib_1.CustomEvent {
    ...
    */
    constructor(config = {}) {
        super();
        if (!config.httpAgent) {
            config.httpAgent = new http_1.Agent({ keepAlive: true });
        }
        if (!config.httpsAgent) {
            config.httpsAgent = new https_1.Agent({ keepAlive: true, rejectUnauthorized: false });
        }
        if (!config.timeout) {
    ...
    errorHandler(error) {
        //event
        this.emit('error', {
            errorCode: 'RiotLocal_Request_Error',
    ...
    * @returns {Promise<RiotLocalAxios<any>>}
    */
    get(url, config = {}) {
