...
const __command: ICommandHandler.File = {
    ...
    async execute({ interaction, language, apiKey, createdTime }) {
        ...
        if (
            TheParty.data.message === "Party does not exist" ||
            TheParty.data.errorCode === "PARTY_DNE"
        ) {
            return {
        ...
        const Party_QueueID: string = QueueId.fromString(
            TheParty.data.MatchmakingData.QueueID as QueueId.Identify
        );
        ...
        sendMessageArray.push(new EmbedBuilder().setColor(`#0099ff`).setTitle(`Members`));

        for (const member of AllMembers) {
            const ThatPlayer = await WebClient.DisplayNameService.fetchPlayers(
                member.PlayerIdentity.Subject
            );
            const ThatPlayerArg = (
                ThatPlayer.data as Array<{
                    Subject: string;
                    GameName: string;
                    TagLine: string;
                }>
            ).find((player) => (player.Subject = member.Subject));
