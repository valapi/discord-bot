...
const __command: ICommandHandler.File = {
    ...
    async execute({ interaction, language, apiKey, createdTime }) {
        ...
        const ThisRank = (
            (await WebClient.MMR.fetchCompetitiveUpdates(puuid)).data.Matches as Array<any>
        )
            .filter((match) => Number(match.RankedRatingEarned) !== 0)
            .at(0);
        ...
        for (const _rank of AllRanks.data.data) {
            for (const _tier of _rank.tiers) {
                if (_tier.tier == ThisRank.TierAfterUpdate) {
                    ...
                    Rank_Color = String(_tier.backgroundColor).substring(
                        0,
                        _tier.backgroundColor.length - 2
                    );
                    break;
                }
            }
