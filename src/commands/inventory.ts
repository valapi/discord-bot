...
import { arrayRandomIndex } from "../utils/random";

export default new Command(new SlashCommandBuilder().setName("inventory").setDescription("collection"), async (interaction) => {
    ...
    if (saved) {
        ...
        inventoryEmbed.setThumbnail(sprayIcon[arrayRandomIndex(sprayIcon)] || null);
        inventoryEmbed.addFields({
            name: "Sprays",
