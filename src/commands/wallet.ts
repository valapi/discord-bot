...
export default new Command(new SlashCommandBuilder().setName("wallet").setDescription("balance"), async (interaction) => {
    ...
    if (saved) {
        ...
        const walletEmbed = new EmbedBuilder().setThumbnail(currencies.data.data.at(ThumbnailId)?.displayIcon || null).setColor(Colors.Aqua);
