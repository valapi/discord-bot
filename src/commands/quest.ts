...
export default new Command(
    ...
    async (interaction) => {
        ...
        if (saved) {
                ...
                .setDescription(
                    `Time Remaining: ${bold(
                        time(
                            Math.round(
                                Date.now() / 1000 +
                                    dailyTicket.data.DailyRewards.RemainingLifetimeSeconds
                            ),
            ...
            const milestoneScore = dailyTicket.data.DailyRewards.Milestones.map(
                (milestone, index) => {
                    checkpointEmbed.addFields({
                        name: `Milestone ${index + 1}`,
                        value: `${["🟩", "🟩", "🟩", "🟩"]
                            .slice(0, milestone.Progress)
                            .join(" ")} ${["🟥", "🟥", "🟥", "🟥"]
            ...
            const milestoneAverageScore =
                milestoneScore.reduce((previous, current) => previous + current) /
                milestoneScore.length;
            ...
            for (const mission of contract.data.Missions) {
                ...
                const missionProgress =
                    mission.Objectives[missionData.data.data.objectives[0].objectiveUuid];

                missionEmbed.addFields({
                    name: missionData.data.data.title,
                    value: `${
                        missionProgress === missionData.data.data.progressToComplete
                            ? missionProgress
                            : bold(missionProgress.toString())
